...
router.get('/', authenticateToken, async (req, res) => {
  ...
  const formatted = (records || []).map(r => ({
    ...
    household_code: r.households ? r.households.household_code : null,
    released_by_name: r.users ? r.users.full_name : null
  }));
  res.json(formatted);
});
...
router.post('/', authenticateToken, async (req, res) => {
  ...
  const { data, error } = await supabase
    .from('assistance_records')
    .insert({ beneficiary_id, assistance_type, amount, distribution_date, program_source, reference_number, status: status || 'RELEASED', released_by: req.user.id, remarks: remarks || null })
    .select()
    .single();
